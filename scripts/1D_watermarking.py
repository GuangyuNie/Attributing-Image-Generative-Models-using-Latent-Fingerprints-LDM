...
def get_editing_direction():
    ...
    shift = opt.shift
    v,u = get_subset_pc(pc,opt.key_len,shift)

    var_v, var_u = get_subset_pc(var, opt.key_len, shift)
...
def get_batched_latent(proj,proj_v ):
    v,u,mean, var_v, var_u = get_editing_direction()
...
@torch.no_grad()
def make_convolutional_sample(model, proj, proj_v,
    ...
    with model.ema_scope("Plotting"):
        ...
        samples, latent, key = get_batched_latent(proj, proj_v, )
    ...
    for i, img in enumerate(imgs):
        ...
        for alpha in sample:
            ...
            for i in tqdm(range(opt.steps)):
                model.zero_grad()
                optimizer.zero_grad()
                estimated_latent = get_latent_from_key(alpha, sigmoid(key), u, v, mean)
